...
function ResourceView() {
  ...
  const embedUrl = cat === "video" ? getYouTubeEmbedUrl(item?.video_url) : null;

  return (
    <div style={{ background: "#070808", minHeight: "100vh", color: "#fff" }}>
      {/* Top bar — matches login/register style */}
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            color: "#aeb4c1", fontSize: "15px", fontWeight: 500,
        ...
        {item && (
          <>
            ...
            <h1 style={{
              ...
              fontWeight: 500, lineHeight: 1.3, marginBottom: "16px",
            }}>
              {item.title}
            </h1>
            ...
            {cat === "video" && !embedUrl && item.video_url && (
              <p style={{ color: "#f08f8f", marginBottom: "24px" }}>Could not embed video. <a href={item.video_url} target="_blank" rel="noreferrer" style={{ color: "#b89381" }}>Open on YouTube</a></p>
            )}

            {/* Audio player */}
            {cat === "audio" && item.video_url && (
              <div style={{ marginBottom: "36px", padding: "24px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
                <audio
                  controls
                  style={{ width: "100%" }}
                  src={item.video_url}
            ...
            {/* Body */}
            <div style={{
              color: "#d9d9d9", fontSize: "1.05rem", lineHeight: 1.85,
              whiteSpace: "pre-wrap",
            }}>
              {item.body}
            </div>
